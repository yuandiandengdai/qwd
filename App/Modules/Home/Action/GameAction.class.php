...
class GameAction extends Action{
    ...
    public function question(){
        ...
        D('Desk')->where(array('rid' => $_SESSION['rid'], 'tid' => $_SESSION['tid']))->setField('winner_counter', 0); //游戏结束，未答题数清空

        D('Desk')->where(array('rid' => $_SESSION['rid'], 'tid' => $_SESSION['tid']))->setField('again', 3);
        //游戏结束，未答题数清空
        ...
        if (IS_GET) {
            ...
            if ($memberAnswer == $answer) { //回答正确
                ...
                if (intval($id) < (strlen($number) - 1)) {
                    ...
                    D('Desk')->where(array('rid' => $_SESSION['rid'], 'tid' => $_SESSION['tid']))->setField('member_one_counter', 0); //游戏结束，未答题数清空
                    D('Desk')->where(array('rid' => $_SESSION['rid'], 'tid' => $_SESSION['tid']))->setField('member_two_counter', 0); //游戏结束，未答题数清空
                    D('Desk')->where(array('rid' => $_SESSION['rid'], 'tid' => $_SESSION['tid']))->setField('member_three_counter', 0); //游戏结束，未答题数清空
    ...
    public function again(){
        ...
        if (IS_POST) {
            ...
            if ($uid) {
                ...
                $data['message'] = '再来一局';
                $member->add($data);
                D('Desk')->where(array('rid' => $_SESSION['rid'], 'tid' => $_SESSION['tid']))->setInc('again');
                $number =  D('Desk')->where(array('rid' => $_SESSION['rid'], 'tid' => $_SESSION['tid']))->getField('again');
                if($number == 3){
                    $this->ajaxReturn(200); //人数够3人
                }else{
                    $this->ajaxReturn(412); //人数不够3人
                }
            } else if($uid == 0){
                $member = D('Message');
    ...
    public function once(){
        ...
        header("Content-Type: text/event-stream");
        header("Cache-Control: no-cache");
        $id = D('Desk')->where(array('rid' => $_SESSION['rid'], 'tid' => $_SESSION['tid']))->getField('again');
        echo 'data:'.intval($id)."\n\n";
        @ob_flush();
        @flush();
    }
